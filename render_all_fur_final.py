...
for char in letters:
    ...
    # Increase bounces to let light penetrate dense fur
    ...
    if principled:
        # Bright pink
        principled.inputs["Base Color"].default_value = (0.9, 0.45, 0.6, 1.0)
    ...
    # Reduced clumping prevents harsh dark spikes
    psettings.clump_factor = 0.35
    ...
    # KEEP emitter rendering, but it has the bright pink material!
    text_obj.show_instancer_for_render = True
    
    # Lighting setup: add a massive front light to blast away shadows
    bpy.ops.object.light_add(type='AREA', location=(0, 0, 4))
    light_front = bpy.context.active_object
    light_front.data.energy = 2000
