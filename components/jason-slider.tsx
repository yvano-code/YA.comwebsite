...
export function JasonSlider({ items, className }: { items: Project[], className?: string }) {
  ...
  const renderMedia = (item: Project, idx: number, isBackground: boolean) => {
    // ONLY render the heavy iframe for the currently active slide, or the one we are transitioning away from.
    // This prevents the browser from crashing or lagging by loading 14+ videos at once!
    const isBgActive = viewMode === 'list'
      ? (hoveredListIndex !== null ? idx === hoveredListIndex : false)
      : idx === activeIndex;

    const isVisible = isBackground
      ? (isBgActive || idx === activeIndex || idx === prevIndex)
      : (idx === activeIndex || idx === prevIndex);
    ...
    if (isVideoLink) {
      return (
        <video
          ...
          autoPlay
          muted={isBackground ? true : isMuted}
          loop
          playsInline
    ...
    if (embedUrl) {
      ...
      if (isYouTube) {
        ...
        if (!isBackground && !isMuted) {
          finalUrl = finalUrl.replace('mute=1', 'mute=0');
        }
      } else if (isVimeo) {
        finalUrl = `${embedUrl}&loop=1#t=${startTime}s`;
        if (!isBackground && !isMuted) {
          finalUrl = finalUrl.replace('background=1', 'background=0&muted=0&autoplay=1');
        }
      }
      ...
      return (
        <motion.iframe
          ...
          className={`w-full h-full object-cover border-0 pointer-events-none ${scaleClass}`}
        />
      );
    }
    return null;
        ...
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center pointer-events-none gap-6">
          ...
          >
              ...
              >
                {items.map((item, idx) => (
                  <div key={idx} className="relative w-full h-full flex-none bg-black">
                    {(idx === activeIndex || idx === prevIndex) && item.href ? (
            ...
            <div className={`absolute -top-6 left-0 right-0 flex justify-between items-center text-[8px] font-mono tracking-[0.2em] text-[#eae3d9] uppercase z-40 transition-opacity duration-300 ${isExpanded ? 'opacity-0' : 'opacity-100'}`}>
              ...
              <div
                className="absolute left-1/2 -translate-x-1/2 flex justify-center items-center h-4 cursor-pointer pointer-events-auto text-[17px] tracking-normal normal-case"
